...
# ============================================================
# HELPER FUNCTION FOR COMPATIBLE RERUN
# ============================================================
def rerun():
...
dark_mode_css = """
    ...
    /* Fixed radio button selection styling */
...
# ============================================================
# 3D GENERATOR - FULLY FIXED
# ============================================================
def generate_3d_view(pretension, shape_type, support_system, supports, span, rise, laa):
    """Generate 3D visualization with all parameters - FULLY FIXED"""
    ...
    if support_system == "2_point":
        # IMPROVED: Dynamic width based on LAA
        base_width = 3.0 * (laa / 10.0)  # Scale width with LAA
        ...
        # FIXED: Support points now match beam ends
        support_y = base_width  # Use the same width as beams
        fig.add_trace(go.Scatter3d(
            x=[-span/2, span/2],
        ...
        # Hypar surface with rise - ensure positive values
        ...
        z_top = rise * np.maximum(0, (1 - ((2 * (x_top - A["x"]) / span_x) - 1)**2) * (1 - ((2 * (y_top - A["y"]) / width_y) - 1)**2))
        fig.add_trace(go.Scatter3d(
        ...
        z_bot = rise * np.maximum(0, (1 - ((2 * (x_bot - A["x"]) / span_x) - 1)**2) * (1 - ((2 * (y_bot - A["y"]) / width_y) - 1)**2))
        fig.add_trace(go.Scatter3d(
        ...
        z_left = rise * np.maximum(0, (1 - ((2 * (x_left - A["x"]) / span_x) - 1)**2) * (1 - ((2 * (y_left - A["y"]) / width_y) - 1)**2))
        fig.add_trace(go.Scatter3d(
        ...
        z_right = rise * np.maximum(0, (1 - ((2 * (x_right - A["x"]) / span_x) - 1)**2) * (1 - ((2 * (y_right - A["y"]) / width_y) - 1)**2))
        fig.add_trace(go.Scatter3d(
 ...
 "        edge_pairs = [(A, B), (B, D), (D, C), (C, A)]
    ...
    fig.update_layout(
        scene=dict(
            ...
            zaxis=dict(color='#b0c4de', gridcolor='#1a2a3a', range=[0, max(10, rise*1.2)]),
            bgcolor='#0a0e17',
