...
def get_sections_by_type(section_type):
    type_map = {
        "CHS": "CHS", "SHS": "SHS", "RHS": "RHS",
        "I-Beam": "I-Beam", "Angle": "Angle", "Channel": "Channel",
    }
...
def generate_bill_of_quantities(design_results, materials):
    ...
    if "members" in design_results:
        ...
        items.append({
            "item": "Truss Chords", "section": unified_type, "qty": 2,
            "unit": "pcs", "length_per_pc": 10.0, "total_length": 20.0,
            "total_weight": 20.0 * 40, "notes": "Top and Bottom chords",
        })
        items.append({
            "item": "Truss Webs", "section": unified_type, "qty": 10,
            "unit": "pcs", "length_per_pc": 1.0, "total_length": 10.0,
            "total_weight": 10.0 * 15, "notes": "Vertical and Horizontal webs",
        })
    elif "beams" in design_results:
        beam = design_results["beams"]["main"]
        items.append({
            "item": "Main Curved Beam", "section": beam.get("section", "N/A"),
            "qty": 2, "unit": "pcs", "length_per_pc": 15.0, "total_length": 30.0,
            "total_weight": 30.0 * 13.5, "notes": "Curved beam members",
        })
    if "secondary_beams" in design_results:
        sec = design_results["secondary_beams"]
        items.append({
            "item": "Secondary Beams", "section": sec.get("section", "N/A"),
            "qty": sec.get("num_purlins", 0), "unit": "pcs", "length_per_pc": 8.0,
            "total_length": sec.get("total_length", 0),
            "total_weight": sec.get("total_weight", 0), "notes": "Purlins",
        })
...
def generate_curved_beam_3d(params, materials=None, curve_type="parabolic"):
    ...
    fig.add_trace(go.Scatter3d(x=x, y=y1, z=z_beam, mode="lines", line=dict(color="#FF6B6B", width=4), showlegend=False))
    fig.add_trace(go.Scatter3d(x=x, y=y2, z=z_beam, mode="lines", line=dict(color="#FF6B6B", width=4), showlegend=False))
    ...
    for i, x_pos in enumerate(x):
        ...
        for j, v_val in enumerate(np.linspace(0, 1, num_points)):
            ...
            Y_surf[i, j] = y_pos
            Z_surf[i, j] = z_pos
    fig.add_trace(go.Surface(x=X_surf, y=Y_surf, z=Z_surf, colorscale=[[0, "#2a3a5f"], [0.5, "#4a7a9c"], [1, "#6ab0d4"]], opacity=0.5, showscale=False, name="Membrane"))
    ...
    if design_results and "secondary_beams" in design_results:
        ...
        if num_purlins > 0:
            ...
            for px in purlin_positions:
                ...
                y_start = y1[idx] * 0.9
                y_end = y2[idx] * 0.9
                fig.add_trace(go.Scatter3d(x=[px, px], y=[y_start, y_end], z=[z_at_p, z_at_p], mode="lines", line=dict(color="#e67e22", width=3, dash="dash"), showlegend=False))
    ...
    if materials:
        ...
        for bx in bracing_x:
            ...
            if tie_system == "rigid":
                fig.add_trace(go.Scatter3d(x=[x1, anchor_x], y=[anchor_y, anchor_y], z=[z_pt, 0], mode="lines", line=dict(color="#f1c40f", width=3), showlegend=False))
            else:
                fig.add_trace(go.Scatter3d(x=[x1, anchor_x], y=[anchor_y, anchor_y], z=[z_pt, 0], mode="lines", line=dict(color="#3498db", width=2, dash="dot"), showlegend=False))
    fig.update_layout(
        scene=dict(
            ...
            bgcolor="#0a0e17",
            camera=dict(eye=dict(x=cam_eye_x, y=cam_eye_y, z=cam_eye_z), up=dict(x=0, y=0, z=1)),
            dragmode="turntable", hovermode="closest",
        ),
        paper_bgcolor="#0a0e17", margin=dict(l=0, r=0, b=0, t=0),
        autosize=True, width=None, height=600,
    )
    return fig
...
def generate_saddle_span(params, materials=None):
    ...
    if materials:
        ...
        for bx in bracing_x:
            ...
            fig.add_trace(go.Scatter3d(x=[x1, anchor_x], y=[y1_pt, anchor1_y], z=[z_pt, 0], mode="lines", line=dict(color=line_color, width=2 if tie_system == "cable" else 3, dash=dash_style), showlegend=False))
            fig.add_trace(go.Scatter3d(x=[x1, anchor_x], y=[y2_pt, anchor2_y], z=[z_pt, 0], mode="lines", line=dict(color=line_color, width=2 if tie_system == "cable" else 3, dash=dash_style), showlegend=False))
    fig.update_layout(
        scene=dict(
            xaxis_title="Span (m)", yaxis_title="Width (m)", zaxis_title="Height (m)",
            xaxis=dict(color="#b0c4de", gridcolor="#1a2a3a"),
            ...
            zaxis=dict(color="#b0c4de", gridcolor="#1a2a3a"),
            bgcolor="#0a0e17",
            camera=dict(eye=dict(x=1.6, y=1.6, z=1.2), up=dict(x=0, y=0, z=1)),
            dragmode="turntable", hovermode="closest",
        ),
        paper_bgcolor="#0a0e17", margin=dict(l=0, r=0, b=0, t=0),
        autosize=True, width=None, height=600,
...
def generate_geodesic_dome_3d(params):
    ...
    if nodes:
        ...
        fig.add_trace(go.Scatter3d(x=xs, y=ys, z=zs, mode="markers", marker=dict(color="#f39c12", size=3), name="Nodes"))
        ...
        if len(nodes) < 200:
            for i in range(len(nodes)):
                for j in range(i + 1, len(nodes)):
                    ...
                    if dist < threshold:
                        fig.add_trace(go.Scatter3d(x=[nodes[i][0], nodes[j][0]], y=[nodes[i][1], nodes[j][1]], z=[nodes[i][2], nodes[j][2]], mode="lines", line=dict(color="#4a7a9c", width=2), showlegend=False))
    fig.update_layout(
        scene=dict(
...
def render_dashboard():
    ...
    with col1:
        ...
        st.markdown(intelligent_html, unsafe_allow_html=True)
    ...
    cols = st.columns(4)
    with cols[0]:
        c0_html = '<div class="dash-card"><div class="icon">#</div><div class="value">' + str(len(projects)) + '</div><div class="label">Saved Projects</div></div>'
        st.markdown(c0_html, unsafe_allow_html=True)
    with cols[1]:
        st.markdown('<div class="dash-card"><div class="icon">#</div><div class="value">27</div><div class="label">Structure Types</div></div>', unsafe_allow_html=True)
    with cols[2]:
...
def render_catalog():
    ...
    with col1:
        ...
        st.markdown(parab_html, unsafe_allow_html=True)
            ...
            if i + j < len(items):
                ...
                with cols[j]:
                    card_html = (
                        ...
                        + data.get("category", "General") + '</div></div>'
                    )
...
def render_bq_page():
    ...
    for item in bq["items"]:
        lpp = item.get("length_per_pc", 0)
        tl = item.get("total_length", 0)
        tw = item.get("total_weight", 0)
        row = {
            "Item": item.get("item", "N/A"),
            "Specification": item.get("section") or item.get("material") or item.get("type") or "N/A",
            "Qty": item.get("qty", "-"),
            "Unit": item.get("unit", "-"),
            "Length/pc (m)": ("%.1f" % lpp) if isinstance(lpp, (int, float)) else "-",
            "Total Length (m)": ("%.1f" % tl) if isinstance(tl, (int, float)) else "-",
            "Weight (kg)": ("%.1f" % tw) if isinstance(tw, (int, float)) else "-",
...
def render_reports():
    ...
    with export_cols[0]:
        if st.button("CSV", key="export_csv", use_container_width=True):
            ...
            st.download_button(label="Download CSV", data=csv_data, file_name="Results_" + st.session_state.project_info.get("reference", "project") + ".csv", mime="text/csv", key="csv_download_btn")
            st.success("CSV ready!")
    with export_cols[1]:
        if st.button("JSON", key="export_json", use_container_width=True):
            json_data = export_to_json(design_results)
            st.download_button(label="Download JSON", data=json_data, file_name="Results_" + st.session_state.project_info.get("reference", "project") + ".json", mime="application/json", key="json_download_btn")
            st.success("JSON ready!")
    st.divider()
    st.subheader("Design Summary")
    health_html = '<div class="health-score"><div class="big">100%</div><div class="sub">ALL COMPONENTS HEALTHY - Design is structurally sound</div></div>'
    ...
    if span_rules:
        for msg in span_rules.get("info_messages", []):
            st.info(msg)
        for msg in span_rules.get("warning_messages", []):
            st.warning(msg)
    if "members" in design_results:
        unified_type = design_results.get("unified_section_type", "CHS")
...
def render_workspace():
    ...
    with col_left:
        ...
        if typology in ["parabolic_beam", "circular_beam"]:
            ...
            selected_curve_label = st.selectbox("Curve Type", curve_labels, index=curve_idx, disabled=st.session_state.locked, key="curve_type_workspace")
        ...
        materials["material_type"] = st.selectbox("Member Material", material_types, index=material_types.index(current_material), disabled=st.session_state.locked, key="material_type_workspace")
        section_types = ["CHS", "SHS", "RHS", "I-Beam", "Angle", "Channel"]
        current_section_type = materials.get("section_type", "CHS")
        materials["section_type"] = st.selectbox("Section Shape", section_types, index=section_types.index(current_section_type), disabled=st.session_state.locked, key="section_type_workspace")
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        selected_member_label = st.selectbox("Member Type", member_labels, index=member_idx, disabled=st.session_state.locked, key="member_type_workspace")
        ...
        if materials["member_type"] in ["planar_truss", "space_truss"]:
            depth_mode = st.radio("Truss Depth", ["Auto-Calculate", "Manual Input"], index=0 if materials.get("truss_depth_mode") == "auto" else 1, disabled=st.session_state.locked, key="truss_depth_mode_workspace")
            materials["truss_depth_mode"] = "auto" if depth_mode == "Auto-Calculate" else "manual"
            if depth_mode == "Manual Input":
                materials["truss_depth_manual"] = st.number_input("Truss Depth (m)", 0.5, 10.0, materials.get("truss_depth_manual", 1.0), 0.1, disabled=st.session_state.locked, key="truss_depth_manual_workspace")
        ...
        if typology in ["parabolic_beam", "circular_beam"]:
            ...
            if span >= 20.0 or apex >= 20.0:
                ...
                selected_joint_label = st.selectbox("Connection Type", joint_labels, index=joint_idx, disabled=st.session_state.locked, key="joint_type_workspace")
        ...
        if materials["member_type"] in ["planar_truss", "space_truss"]:
            ...
            st.markdown(truss_info_html, unsafe_allow_html=True)
            ...
            selected_truss_label = st.selectbox("Truss Type", truss_labels, index=truss_idx, disabled=st.session_state.locked, key="truss_type_workspace")
            truss_keys = ["warren", "pratt", "howe", "vierendeel"]
            materials["truss_type"] = truss_keys[truss_labels.index(selected_truss_label)]
            materials["num_bays"] = st.number_input("Number of Bays", min_value=1, max_value=20, value=materials.get("num_bays", 3), step=1, disabled=st.session_state.locked, key="num_bays_workspace")
        ...
        if typology in ["parabolic_beam", "circular_beam", "saddle_span", "clear_span_tent", "tensile_membrane", "shade_structure"]:
            ...
            materials["fabric_type"] = st.selectbox("Fabric Material", fabric_options, index=fabric_options.index(materials.get("fabric_type", "PVC-coated Polyester")), disabled=st.session_state.locked, key="fabric_type_workspace")
            materials["fabric_sag"] = st.slider("Fabric Sag (%)", 5, 50, materials.get("fabric_sag", 30), 5, disabled=st.session_state.locked, key="fabric_sag_workspace")
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        if typology in ["parabolic_beam", "circular_beam", "saddle_span", "clear_span_tent", "tensile_membrane", "cable_net", "cable_stayed"]:
            ...
            selected_tie = st.radio("Tie-down System", tie_options, index=tie_options.index(current_tie), disabled=st.session_state.locked, key="tie_down_system_workspace")
            ...
            if materials["tie_down_system"] == "cable":
                ...
                materials["cable_type"] = st.selectbox("Cable Type", cable_options, index=cable_idx, disabled=st.session_state.locked, key="cable_type_workspace")
        ...
        if typology in ["parabolic_beam", "circular_beam", "saddle_span"]:
            if params and "B" in params and "LAA" in params:
                ...
                for msg in rules.get("info_messages", []):
                    st.info(msg)
                for msg in rules.get("warning_messages", []):
                    st.warning(msg)
    ...
    with col_right:
        ...
        if "design_results" in st.session_state and st.session_state.design_results:
            ...
            health_html = '<div class="health-score"><div class="big">100%</div><div class="sub">ALL COMPONENTS HEALTHY</div></div>'
            st.markdown(health_html, unsafe_allow_html=True)
            span_rules = design_results.get("span_rules", {})
            if span_rules:
                for msg in span_rules.get("info_messages", []):
            ...
            if "members" in design_results:
                unified_type = design_results.get("unified_section_type", "CHS")
                is_3d = design_results.get("is_3d", False)
                truss_hdr = '<div class="sdse-card"><div class="card-title">Truss Members <span class="badge badge-unified">ALL ' + unified_type + '</span></div>'
                st.markdown(truss_hdr, unsafe_allow_html=True)
                ...
                if beam:
                    ...
                    if is_standard:
                        member_html = (
                            ...
                            + '</div>'
                            + '<div><div style="color: #2ecc71; font-weight: 700;">PASS</div></div>'
                            + '</div>'
                        )
                        ...
                        member_html = (
                            ...
                            + '<div class="section-detail">Type: ' + section_type + '</div>'
                            + '</div>'
                            + '<div><div style="color: #f39c12; font-weight: 700;">CHECK</div></div>'
                            + '</div>'
                        )
                        st.markdown(member_html, unsafe_allow_html=True)
                        if beam.get("closest"):
            ...
            if "secondary_beams" in design_results:
                sec = design_results["secondary_beams"]
                sec_hdr = '<div class="sdse-card"><div class="card-title">Secondary Beams <span class="badge badge-secondary">PURLINS</span></div>'
                st.markdown(sec_hdr, unsafe_allow_html=True)
                st.caption("**Section:** " + sec.get("section", "N/A") + " | Count: " + str(sec.get("num_purlins", 0)) + " | Spacing: %.1fm" % sec.get("spacing", 0))
                st.caption("Total Length: %.1fm | Weight: %.1fkg" % (sec.get("total_length", 0), sec.get("total_weight", 0)))
                st.markdown("</div>", unsafe_allow_html=True)
            ...
            if "rigid_ties" in design_results:
                ties = design_results["rigid_ties"]
                tie_hdr = '<div class="sdse-card"><div class="card-title">Rigid Tie-downs <span class="badge badge-tie">TIES</span></div>'
                st.markdown(tie_hdr, unsafe_allow_html=True)
                st.caption("**Section:** " + ties.get("section", "N/A") + " | Count: " + str(ties.get("num_ties", 0)) + " | Force per tie: %.1fkN" % ties.get("force_per_tie", 0))
                st.caption("Total Length: %.1fm | Weight: %.1fkg" % (ties.get("total_length", 0), ties.get("total_weight", 0)))
                st.markdown("</div>", unsafe_allow_html=True)
            ...
            if "cables" in design_results:
                cables = design_results["cables"]
                cab_hdr = '<div class="sdse-card"><div class="card-title">Cables <span class="badge badge-cable">CABLES</span></div>'
                st.markdown(cab_hdr, unsafe_allow_html=True)
                st.caption("**Type:** " + cables.get("type", "N/A") + " | Diameter: " + str(cables.get("diameter", 0)) + "mm")
                st.caption("Force per cable: %.1fkN | Utilization: %.0f%%" % (cables.get("force_per_cable", 0), cables.get("utilization", 0) * 100))
                st.markdown("</div>", unsafe_allow_html=True)
